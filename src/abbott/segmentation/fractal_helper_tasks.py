...
# Zurich.
"""
Functions to use masked loading of ROIs before/after processing. Adapted from 
fractal-tasks-core masked_loading wrapper
"""
import logging
...
def _preprocess_input(
    ...
    *,
    ...
    """Preprocess a three-dimensional input to stardist_segmentation task.

    **NOTE**: Adapted from cellpose_segmentation task together with
    masked_loading_wrapper_segmentation and _postprocess_output to make sure
    ...
    **NOTE 2**: The pre/post-processing functions and the
    ...
    masking_label_path = str(Path(ROI_table_path).parent / label_relative_path / f"{level}")
...
def _postprocess_output(
    *,
    ...
    """
    Postprocess stardist output, mainly to restore its original background.
